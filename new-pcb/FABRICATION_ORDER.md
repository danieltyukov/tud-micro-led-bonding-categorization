...
| `tud-microled-v2-aisler-bom.csv` | Beagle assembly — BOM (with MPN + DNP per part) |
...
The BOM is generated by `new-pcb/tools/gen_aisler_bom.py` which reads the
pos file and maps every footprint to its manufacturer part number. Re-run
this tool after any board change to regenerate.
